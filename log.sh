...
# Carica START_HOUR e END_HOUR dal file di configurazione
START_HOUR=$(awk -F'=' '/^START_HOUR/{print $2}' "$CONFIG" | tr -d ' ')
END_HOUR=$(awk -F'=' '/^END_HOUR/{print $2}' "$CONFIG" | tr -d ' ')
...
case "$1" in
    ...
    # ------------------------------------------------------------------
    # IP: Log di un indirizzo IP specifico
    # ------------------------------------------------------------------
    -i|--ip)
        IP="$2"
        if [[ -z "$IP" ]]; then
    ...
    # ------------------------------------------------------------------
    # DATE: Log di una data specifica
    # ------------------------------------------------------------------
    -d|--date)
        DATE="$2"
        if [[ -z "$DATE" ]]; then
    ...
    # ------------------------------------------------------------------
    # DATES: Log in un intervallo di date
    # ------------------------------------------------------------------
    -ds|--dates)
        DATE1="$2"
        DATE2="$3"
    ...
    # ------------------------------------------------------------------
    # DATE-TIME: Log di una data in un intervallo orario
    # ------------------------------------------------------------------
    -dt|--date-time)
        DATE="$2"
        START_HR="$3"
        END_HR="$4"
        if [[ -z "$DATE" || -z "$START_HR" || -z "$END_HR" ]]; then
            echo "$USAGE"
            exit 1
        fi
        if ! validate_date "$DATE"; then
        ...
        OUT="$OUT_FILE/log_date_hour_$DATE_${S_PAD}-${E_PAD}.log"
        awk -F"|" -v d="$DATE" -v s="$S_PAD" -v e="$E_PAD" '{h = substr($2,1,2) + 0 } $1 == d && h >= s && h < e' "$LOG_FILE" > "$OUT"
        if [[ -s "$OUT" ]]; then
            echo "Voci di log per la data $DATE tra le $S_PAD:00 e le $(($E_PAD - 1)):59 salvate in $OUT_FILE come log_date_hour_$DATE_${S_PAD}-${E_PAD}.log"
        else
            rm -f "$OUT"
